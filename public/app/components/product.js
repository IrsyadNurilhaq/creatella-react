...
class Product extends React.Component{
    ...
    showData = (data) => {
        ...
        const showData = data.map((data,index) => {
            let dollars = helper.convertDollarFormat(data.price);
            let diff = helper.calculateDate(data.date);
            return(
                <Col lg={3} md={4} xs={6} className="margin-15" key={index}>
                    <div className="card" ref={this.myRef}>
                        ...
                        <Card.Footer>
                            <small className="text-muted"> {diff <= 7 ? diff + " days ago" : helper.formatDate(data.date)} </small>
                        </Card.Footer>
                    </div>
                </Col>
            );
